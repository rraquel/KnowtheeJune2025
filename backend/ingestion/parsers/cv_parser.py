                        ...
                        for pattern in degree_patterns:
                            ...
                            if match:
                                if "in" in degree_line.lower():
                                    ...
                                    degree = match.group(1).strip()
                                    # Try to extract field from the rest of the line
                                    field_match = re.search(r'(?:in|of)\s+([^,]+)', degree_line)
                                    if field_match:
        ...
        # Experience parsing
            ...
            if date_match:
                ...
                if exp_buffer:
                    ...
                    existing_exp = next((exp for exp in data["experiences"] 
                                      if exp["title"] == title.strip() 
                                      and exp["company"] == company.strip()
                                      and exp["start_date"] == start_date
                                      and exp["end_date"] == end_date), None)
